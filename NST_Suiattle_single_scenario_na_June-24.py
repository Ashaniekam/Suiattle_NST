...
c_ti = model_time.ctime(os.path.getctime("NST_Suiattle_single_scenario_na_June-24.py"))
...
#XXX %%
  #Since Steady State  ################################ INCOMPLETE: current work
#   # Function to shorten and normalize the array
# def since_steady_state(variable):
#     if pulse_time-1 < 0 or pulse_time-1 >= variable.shape[1]:
#         raise IndexError(f"steady_state_index {pulse_time-1} is out of bounds for variable with shape {variable.shape}")
...
#     shortened_variable = variable[:, pulse_time-1:]

#       # Normalize by steady state values for each variable
#     steady_state_values = variable[:, pulse_time-1]  # Steady state values
...
# variables = [
#     {
#         "variable": num_active_parcels_each_link_DS ,
#         "name": "sorted_num_active"
#     },
#     {
#         "variable": volume_at_each_link_DS,
#         "name": "sorted_volume_at_each_link"
#     },
#     {
#         "variable": D_mean_each_link_DS,
...
#     normalized_variables[var["name"]] = since_steady_state(var["variable"], pulse_time-1)
...
plt.figure()
plt.pcolor(time_array, dist_upstream, num_pulse_each_link_DS)
...
plt.figure()
plt.pcolor(time_array, dist_upstream, num_total_parcels_each_link_DS)
...
plt.figure()
num_active_pulse_nozero = num_active_pulse_DS.copy()
...
plt.figure()
vol_pulse_nozero = volume_pulse_at_each_link_DS.copy()
...
plt.figure()
vol_nozero = volume_at_each_link_DS.copy()
...
plt.figure()
D_mean_pulse_each_link_nozero= D_mean_pulse_each_link_DS.copy()
...
plt.figure()
D_mean_each_link_nozero= D_mean_each_link_DS.copy()
