...
if scenario == 1:
    ...
    scenario_num = "tumber_correction_2"

# XXX TIME in the model

timesteps = 700 # number of timesteps    

# no_abrasion, shrs_abrasion and abrasion_correction2
#scenario_1_path= os.path.join("C:\Users\longrea\OneDrive - Western Washington University\Thesis\Plots\Scenario 1")
#with_abrasion = ""
#with_abrasion = "tumber_correction_2"
#with_abrasion = "none"

#density_values = "constant"
#density_values = "SHRS proxy"
...
pulse_time = 2
pulse_parcel_vol = 10 
num_pulse_parcels = 29450 #np.round((np.mean(vol_of_channel/2))/pulse_parcel_vol) #980000 
...
for t in range(0, (timesteps*dt), dt):
    ...
    #making a pulse
    if t==dt*pulse_time or t==dt*199: 
        print('making a pulse')
        print('t = ',t, ',timestep = ', t/dt)
        
        pulse_location = [random.randrange(15, 36, 1) for i in range(num_pulse_parcels)] #code from nst_test.py: adding pulse randomly near the fan
        ...
        # Will use constant density for AGU
        new_density = 2200 * np.ones(np.size(newpar_element_id))  # (kg/m3)
        
        #this will be used after agu
        # if scenario <= 4:
        #     new_density = 2200 * np.ones(np.size(newpar_element_id))  # (kg/m3)
        # else:
        #     new_density = (np.random.uniform(1500, 2750, size=(np.size(newpar_element_id)))) * np.ones(np.size(newpar_element_id)) #distribution from SHRS PROXY fron Figure1 in Pfeiffer_et_al_2022: Survival of the strong and dense
        ...
        SHRS_MEAN_ALPHA = np.mean(SHRS)
        SHRS_STDEV_ALPHA = np.std(SHRS)

        distribution = np.random.normal(SHRS_MEAN_ALPHA, SHRS_STDEV_ALPHA, (np.size(newpar_element_id)))


        measured_alphas = 3.0715*np.exp(-0.136*distribution)

        tumbler_2 = 2 * (3.0715*np.exp(-0.136*distribution))
        tumbler_4 = 4 * (3.0715*np.exp(-0.136*distribution))
        
        if scenario == 1 or scenario == 5:
            new_abrasion_rate = 0 * np.ones(np.size(newpar_element_id))
        elif scenario == 2 or scenario == 6:
            new_abrasion_rate = (measured_alphas/1000)* np.ones(np.size(newpar_element_id)) #0.3 % mass loss per kilometer
        elif scenario == 3 or scenario == 7: 
            new_abrasion_rate = (tumbler_2/1000)* np.ones(np.size(newpar_element_id)) #tumbler correction 2
        else: 
    ...
    if t%((timesteps*dt)/n_lines)==0:        
        ...
        plt.xlabel('Distance downstream (km)')
        plt.title("Elevation change per day")
...
#%%
    ...
    n_pulse_remain = n_pulse_on - n_pulse_left
    percent_pulse_remain = (n_pulse_remain/n_pulse_on)*100
...
parcels_dataframe.to_csv('Parcels_data_scenario'+scenario_num+'.csv', sep=',', index=True) 
...
plt.xlabel("Days")
plt.ylabel("Distance from upstream (km)")
plt.savefig('Vol_pluse(Scenario'+scenario_num+ ').png', dpi=700)
plt.show()
...
plt.xlabel("Days")
plt.ylabel("Distance from upstream (km)")
plt.savefig('Dmean_pluse(Scenario'+scenario_num+ ').png', dpi=700)
plt.show()
...
plt.xlabel("Days")
plt.ylabel("Distance from upstream (km)")
plt.savefig('percent_active_pulse(Scenario'+scenario_num+ ').png', dpi=700)
plt.show()
...
plt.xlabel('Days')
plt.ylabel('Distance from upstream (km)')
plt.savefig('TrackElevChange(Scenario'+scenario_num+ ').png', dpi=700)
plt.show()
